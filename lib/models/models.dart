...
class Hotel {
  ...
  Hotel(
      ...
      this.tag = '',
      this.category = HotelCategory.hotel});
  ...
  factory Hotel.fromFirestore(DocumentSnapshot doc) {
    ...
    return Hotel(
        ...
        category: HotelCategory.values.firstWhere(
            (e) => e.name == (d['category'] ?? 'hotel'),
            orElse: () => HotelCategory.hotel));
  }
...
class Flight {
  ...
  factory Flight.fromFirestore(DocumentSnapshot doc) {
    ...
    return Flight(
        ...
        stops: d['stops'] ?? 0,
        flightClass: d['flightClass'] ?? 'Economy');
  }
...
class AppUser {
  ...
  factory AppUser.fromFirestore(DocumentSnapshot doc) {
    ...
    return AppUser(
        ...
        email: d['email'] ?? '',
        photoUrl: d['photoUrl'],
        membershipTier: d['membershipTier'] ?? 'Silver',
        points: d['points'] ?? 0,
        wishlistHotelIds: List<String>.from(d['wishlistHotelIds'] ?? []),
        createdAt: (d['createdAt'] as Timestamp?)?.toDate() ?? DateTime.now());
  }
...
class TravelPackage {
  ...
  factory TravelPackage.fromFirestore(DocumentSnapshot doc) {
    ...
    return TravelPackage(
        ...
        nights: d['nights'] ?? 3,
        image: d['image'] ?? '',
        includes: List<String>.from(d['includes'] ?? []),
        rating: (d['rating'] ?? 0).toDouble());
  }
